...
if __name__ == '__main__':
    ...
    #             yl = 'n_constraints'
    #         case 'mean':
    #             yl = 'ex_memory_mean(MiB)'
    #         case 'max':
    #             yl = 'ex_memory_max(MiB)'
    #         case _:
    #             continue
    ...
    for file in os.listdir(result_folder):
        ...
        match y_label:
            case 'presence':
                yl = 'Frazioni di soluzioni trovate'
            case 'validity':
                yl = 'Frazioni di soluzioni valide'
            case _:
                continue
        ...
        for i, model in enumerate(models):
            ...
            # add bars labels
            for j, val in enumerate(df.loc[model]):
                label = f"{int(val)}" if float(val).is_integer() else f"{val:.5f}"
                plt.text(x[j] + (i - len(models)/2) * bar_width + bar_width/2,
                        val + 0.01,
                        label,
                        ha='center', 
