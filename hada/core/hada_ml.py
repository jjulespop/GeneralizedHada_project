...
def HADA(db : ConfigDB,
    ...
    inputs_and_hyperparams = datasets.expander.get_expanded_ml_input_vars(request.algorithm)
    var_type = datasets.expander.get_expanded_var_type(request.algorithm)
    var_bounds = dict(
        {var : var_bounds[var] for var in var_bounds if var not in str_vars},
    ...
    if request.input_dependent:
        ...
        inputs_types = {var:type for var,type in db.get_type_per_var(request.algorithm).items()
                        if var in inputs.keys()}
        for input_var, type in inputs_types.items():
            if type == 'str':
                enc_categories = datasets.expander.get_encoded_selection(request.algorithm, input_var, inputs[input_var])
    ...
    if sol:
        ...
        for var in set(str_vars.keys()).intersection(db.get_hyperparams(request.algorithm)):
